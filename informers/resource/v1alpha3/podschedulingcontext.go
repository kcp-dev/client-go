...
// PodSchedulingContextClusterInformer provides access to a shared informer and lister for
	...
	return kcpinformers.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
				...
				return client.ResourceV1alpha3().PodSchedulingContexts().List(context.TODO(), options)
			},
			WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.ResourceV1alpha3().PodSchedulingContexts().Watch(context.TODO(), options)
			},
		},
...
func (f *podSchedulingContextClusterInformer) defaultInformer(client clientset.ClusterInterface, resyncPeriod time.Duration) kcpcache.ScopeableSharedIndexInformer {
	return NewFilteredPodSchedulingContextClusterInformer(client, resyncPeriod, cache.Indexers{
...
func (f *podSchedulingContextClusterInformer) Informer() kcpcache.ScopeableSharedIndexInformer {
	return f.factory.InformerFor(&resourcev1alpha3.PodSchedulingContext{}, f.defaultInformer)
}

func (f *podSchedulingContextClusterInformer) Lister() resourcev1alpha3listers.PodSchedulingContextClusterLister {
	return resourcev1alpha3listers.NewPodSchedulingContextClusterLister(f.Informer().GetIndexer())
}

func (f *podSchedulingContextClusterInformer) Cluster(clusterName logicalcluster.Name) upstreamresourcev1alpha3informers.PodSchedulingContextInformer {
	return &podSchedulingContextInformer{
...
type podSchedulingContextInformer struct {
	...
	lister   upstreamresourcev1alpha3listers.PodSchedulingContextLister
}
...
func (f *podSchedulingContextInformer) Lister() upstreamresourcev1alpha3listers.PodSchedulingContextLister {
	return f.lister
}
